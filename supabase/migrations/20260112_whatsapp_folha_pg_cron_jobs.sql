-- WhatsApp (UAZAPI): Folha de Pagamento — fechamento (dia X) + aprovação pendente
...
declare
  jid integer;
begin
  select jobid into jid from cron.job where jobname = 'folha-whatsapp-notificacoes-5min' limit 1;
  if jid is not null then
    perform cron.unschedule(jid);
  end if;

  perform cron.schedule(
    'folha-whatsapp-notificacoes-5min',
    '*/5 * * * *',
    $cmd$
    select net.http_post(
      url := 'https://ubdvtjbitozhkuvvqkxj.supabase.co/functions/v1/whatsapp-folha-notificacoes',
