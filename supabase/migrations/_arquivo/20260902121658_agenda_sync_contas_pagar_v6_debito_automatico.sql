-- v6 = v5 + debito automatico. Conta em debito automatico NAO ganha espelho "Pagar:" na Agenda:
-- nao ha acao manual a lembrar (a visibilidade fica na lista do dia do WhatsApp, com o marcador),
-- e o digest "a pagar hoje" deixa de somar dinheiro que sai sozinho. Espelho ja existente de conta
-- flagada passa a ser orfa (mesma regra de cancelada/finalizada) e e removido no ciclo seguinte.
-- Corpo identico a v5 (20260902042802) fora das duas clausulas marcadas com "v6".
-- Tambem completa o backfill da migration 20260902115559: instancias pendentes de modelos flagados
-- que nasceram antes da flag (Light 164 x3 em set/2026) recebem a flag.

update public.contas_pagar i
   set debito_automatico = true
  from public.contas_pagar m
 where i.recorrente_modelo_id = m.id
   and m.debito_automatico
   and not i.debito_automatico
...
begin
  ...
  with contas as (
    select c.id, c.descricao, c.unidade, c.valor, c.data_vencimento, c.status,
     ...
     where coalesce(c.status,'') not in ('cancelado','finalizado')
       and not c.debito_automatico                                  -- v6: sem espelho para debito automatico
  ...
  -- Orfa = conta inexistente, cancelada, finalizada ou em debito automatico (v6). Sair da janela NAO e orfa.
     ...
     and not exists (
       select 1 from public.contas_pagar c
        where c.id = t.vinculo_id and coalesce(c.status,'') not in ('cancelado','finalizado')
          and not c.debito_automatico                              -- v6
     )
