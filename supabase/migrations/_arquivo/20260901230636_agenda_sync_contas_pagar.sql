...
begin
  ...
  -- Nasce inativo: o orquestrador roda a 1a execucao real, verifica integridade e ativa (Task 4 Step 6).
  -- Via cron.alter_job: "update cron.job" direto e negado (42501, permission denied for table job) —
  -- o role postgres nao tem UPDATE na tabela do pg_cron, so a API security definer.
  select jobid into jid from cron.job where jobname = 'agenda-sync-contas-10min' limit 1;
