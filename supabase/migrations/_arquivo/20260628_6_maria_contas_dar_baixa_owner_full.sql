-- Ajusta a baixa operacional da Maria para permitir owner_full (Alf).
-- Resto do comportamento permanece: registrar pagamento ja realizado, sem mover dinheiro.
...
begin
  ...
  v_obs := concat_ws(E'\n',
    ...
    concat('Baixa registrada pela Maria apos confirmacao de ', v_confirmado_por_nome, ' em ', to_char(now() at time zone 'America/Sao_Paulo', 'DD/MM/YYYY HH24:MI'), '. Sem pagamento real executado pela Maria.'),
...
comment on function public.maria_contas_dar_baixa(
  uuid, date, text, text, text, text, text, text, text, text, text
) is 'Maria operational RPC: da baixa em conta a pagar ja paga pela equipe, com confirmacao humana e auditoria; permite owner_full e nao executa pagamento real.';
