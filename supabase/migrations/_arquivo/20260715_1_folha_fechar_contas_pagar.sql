-- Fase 5 / Fatia C: fechamento da folha gera obrigacoes por conta pagadora.
-- O detalhe fiscal permanece em lancamentos_folha; as contas agregadas ficam fora do DRE por plano.
...
alter table public.contas_pagar
  ...
  check (tipo_lancamento in (
    'unica', 'recorrente', 'parcelada', 'eventual', 'fatura_cartao', 'folha_pagamento'
  ));
...
alter table public.contas_pagar
  ...
  check (fonte_tipo is null or fonte_tipo in (
    'site', 'email', 'pix_fixo', 'banco', 'whatsapp', 'manual', 'cartao', 'folha_pagamento'
  ));
...
create or replace function public.folha_reabrir(
  p_folha_id integer,
  p_ator jsonb default '{}'::jsonb
)
returns jsonb
...
begin
  ...
  if v_role = 'authenticated' then
    ...
    v_ator_tipo := coalesce(nullif(p_ator->>'tipo', ''), 'sistema');
    if v_ator_tipo <> 'sistema' then
      raise exception 'ator.tipo nao permitido para service_role na reabertura da folha.'
        using errcode = '42501';
    end if;
    v_ator_ref := coalesce(nullif(p_ator->>'ref', ''), 'service_role');
    v_ator_nome := coalesce(nullif(p_ator->>'nome', ''), 'Sistema');
  else
    raise exception 'papel nao autorizado para reabrir folha: %', v_role
      using errcode = '42501';
  end if;

  select f.status, f.total_geral
    into v_status, v_total_geral
    from public.folhas_mensais f
   where f.id = p_folha_id
   for update;
  ...
  if v_status <> 'fechada' then
    raise exception 'status da folha deve ser fechada para reabrir; encontrado: %.', v_status;
  end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    into v_contas_ativas
    from public.contas_pagar cp
   where cp.fonte_tipo = 'folha_pagamento'
     and cp.fonte_identificador = p_folha_id::text
     and cp.status <> 'cancelado';

  if jsonb_array_length(v_contas_ativas) = 0 then
    raise exception 'folha % nao possui contas a pagar ativas para reabrir.', p_folha_id;
  end if;
  ...
  if jsonb_array_length(v_bloqueadas) > 0 then
    raise exception 'contas vinculadas nao podem ser canceladas porque ja foram pagas, baixadas ou finalizadas: %.',
      v_bloqueadas;
  end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
   ...
   where cp.fonte_tipo = 'folha_pagamento'
     ...
     and cp.id in (
       select (item->>'id')::uuid
       from jsonb_array_elements(v_contas_ativas) item
     );
  ...
  values (
    v_ator_nome, v_ator_ref, v_numero_hash, v_last4,
    v_ator_tipo, 'folha', v_ator_tipo, v_role,
    'folhas_mensais', 'folha_reabertura', null,
    'REABRIR_FOLHA',
    jsonb_build_object(
      'folha_id', p_folha_id,
      'status', v_status,
      'total_geral', v_total_geral,
      'contas_ativas', v_contas_ativas
    ),
    jsonb_build_object(
      'folha_id', p_folha_id,
      'status', 'aprovada',
...
revoke all on function public.folha_reabrir(integer, jsonb)
  from public, anon, authenticated, maria_operacional, maria_leitura;
grant execute on function public.folha_reabrir(integer, jsonb)
  to authenticated, service_role;

comment on function public.folha_fechar(integer, jsonb) is
  'Fecha uma folha aprovada e totalmente rateada, gerando obrigacoes agregadas por conta pagadora.';

comment on function public.folha_reabrir(integer, jsonb) is
  'Reabre uma folha fechada somente quando todas as obrigacoes vinculadas ainda estao pendentes.';
