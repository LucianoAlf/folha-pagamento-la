  ...
  return (
    ...
    >
      <div className="space-y-4">
        {loading ? <div className="text-muted font-bold">Carregando…</div> : null}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {t.descricao ? <div className="text-xs text-muted font-bold mt-1 line-clamp-2">{t.descricao}</div> : null}
                    </div>
                  </div>
                </button>
              );
            })}
            {templates.length === 0 ? <div className="text-muted font-bold">Nenhum template ativo.</div> : null}
          </div>
        ) : null}
        ...
        {selected ? (
          <div className="rounded-2xl border border-line bg-bg/20 p-4">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              {neededVars.length === 0 ? (
                <div className="text-sm text-muted font-bold">Esse template não precisa de variáveis.</div>
              ) : (
                neededVars.map((k) => (
                  <div key={k}>
            ...
            <div className="mt-4 rounded-2xl border border-line/60 bg-bg/10 p-4">
              ...
              <div className="mt-3 text-[11px] text-muted font-bold">
                Dica: para aparecer em <span className="text-secondary">Planejado</span>, selecione uma data.
              </div>
            </div>
