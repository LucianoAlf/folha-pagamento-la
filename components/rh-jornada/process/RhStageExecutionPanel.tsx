...
export const RhStageExecutionPanel: React.FC<{ process: RhProcess | null; stage: RhStage | null; onStageUpdated?: () => Promise<void> | void; }> = ({ process, stage, onStageUpdated }) => {
  ...
  if (!stage) return <Card className="p-5 border border-strong/50"><div className="text-sm font-bold text-muted">Selecione uma etapa para ver checklist, responsáveis, links e comunicação operacional.</div></Card>;
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5 border border-strong/50">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <div><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Prazo</div><input type="date" value={stageForm.data_limite} onChange={(e) => setStageForm((current) => ({ ...current, data_limite: e.target.value }))} className="w-full rounded-2xl border border-base bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" /></div>
          <div><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Data e hora</div><input type="datetime-local" value={stageForm.agendado_em} onChange={(e) => setStageForm((current) => ({ ...current, agendado_em: e.target.value }))} className="w-full rounded-2xl border border-base bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" /></div>
          <div className="xl:col-span-2"><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Instruções da etapa</div><textarea value={stageForm.instrucoes} onChange={(e) => setStageForm((current) => ({ ...current, instrucoes: e.target.value }))} rows={4} placeholder="Passo a passo, texto-base e orientação da etapa." className="w-full rounded-2xl border border-base bg-bg px-5 py-4 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-accent/40" /></div>
          <div className="xl:col-span-2"><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Modelo de mensagem</div><textarea value={stageForm.modelo_mensagem} onChange={(e) => setStageForm((current) => ({ ...current, modelo_mensagem: e.target.value }))} rows={4} placeholder="Ex.: Olá, {colaborador}. Sua etapa {etapa} foi agendada para {data_hora}. Mentor: {mentor}. Link: {link_reuniao}." className="w-full rounded-2xl border border-base bg-bg px-5 py-4 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-accent/40" /><div className="mt-2 text-xs font-bold text-muted">Os campos entre chaves são preenchidos automaticamente pelo sistema.</div></div>
          <div><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Link de referência</div><input value={stageForm.link_referencia} onChange={(e) => setStageForm((current) => ({ ...current, link_referencia: e.target.value }))} placeholder="Vídeo, playbook, docs" className="w-full rounded-2xl border border-base bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" /></div>
          <div><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Link de reunião</div><input value={stageForm.link_reuniao} onChange={(e) => setStageForm((current) => ({ ...current, link_reuniao: e.target.value }))} placeholder="Meet, Zoom, etc." className="w-full rounded-2xl border border-base bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" /></div>
          <div className="xl:col-span-2"><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Observações internas</div><textarea value={stageForm.observacoes} onChange={(e) => setStageForm((current) => ({ ...current, observacoes: e.target.value }))} rows={3} placeholder="Anotações operacionais e contexto." className="w-full rounded-2xl border border-base bg-bg px-5 py-4 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-accent/40" /></div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_.85fr] gap-6">
        <Card className="p-5 border border-strong/50">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1 space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-3">
                      <input value={item.titulo} disabled={!canOperateStage} onChange={(e) => setChecklist((current) => current.map((entry) => entry.id === item.id ? { ...entry, titulo: e.target.value } : entry))} className="w-full rounded-2xl border border-base bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-success/40" />
                      <label className="flex items-center gap-2 rounded-2xl border border-base bg-bg px-4 py-3 text-sm font-bold text-secondary"><input type="checkbox" checked={item.obrigatorio} disabled={!canOperateStage} onChange={(e) => setChecklist((current) => current.map((entry) => entry.id === item.id ? { ...entry, obrigatorio: e.target.checked } : entry))} className="accent-accent" />Obrigatório</label>
                    </div>
                    <textarea value={item.descricao || ''} disabled={!canOperateStage} onChange={(e) => setChecklist((current) => current.map((entry) => entry.id === item.id ? { ...entry, descricao: e.target.value } : entry))} rows={3} placeholder="Texto operacional, observações e conteúdo reutilizável." className="w-full rounded-2xl border border-base bg-bg px-4 py-3 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-success/40" />
                    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-3">
                      <input value={item.link_url || ''} disabled={!canOperateStage} onChange={(e) => setChecklist((current) => current.map((entry) => entry.id === item.id ? { ...entry, link_url: e.target.value } : entry))} placeholder="Link de apoio" className="w-full rounded-2xl border border-base bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-success/40" />
                      {item.link_url ? <button type="button" onClick={() => window.open(item.link_url!, '_blank', 'noopener,noreferrer')} className="px-4 py-3 rounded-2xl border border-base bg-surface/40 text-secondary font-black hover:bg-surface/60 flex items-center gap-2 transition-all"><ExternalLink className="w-4 h-4" />Abrir</button> : <div />}
                      <div className="flex items-center gap-2">
                        <button type="button" disabled={!canOperateStage || savingItem} onClick={async () => { setSavingItem(true); await run(async () => { await rhJornadaService.updateChecklistItem(item.id, { titulo: item.titulo, descricao: item.descricao || null, link_url: item.link_url || null, obrigatorio: item.obrigatorio, ordem: item.ordem }); await loadData(); }, { success: 'Item do checklist salvo.', error: 'Não foi possível salvar o item do checklist.' }); setSavingItem(false); }} className={cn('px-4 py-3 rounded-2xl font-black text-white transition-all', !canOperateStage || savingItem ? 'bg-surface-3 opacity-60 cursor-not-allowed' : 'bg-success hover:bg-success')}>Salvar</button>
          ...
          <div className="rounded-3xl border border-dashed border-strong bg-surface/20 p-4 space-y-3">
            <input value={newChecklist.titulo} onChange={(e) => setNewChecklist((current) => ({ ...current, titulo: e.target.value }))} placeholder="Novo item do checklist" className="w-full rounded-2xl border border-base bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" />
            <textarea value={newChecklist.descricao} onChange={(e) => setNewChecklist((current) => ({ ...current, descricao: e.target.value }))} rows={3} placeholder="Descrição operacional, texto-base ou passo a passo" className="w-full rounded-2xl border border-base bg-bg px-5 py-4 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-accent/40" />
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-3">
              <input value={newChecklist.link_url} onChange={(e) => setNewChecklist((current) => ({ ...current, link_url: e.target.value }))} placeholder="Link opcional" className="w-full rounded-2xl border border-base bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" />
              <label className="flex items-center gap-2 rounded-2xl border border-base bg-bg px-4 py-3 text-sm font-bold text-secondary"><input type="checkbox" checked={newChecklist.obrigatorio} onChange={(e) => setNewChecklist((current) => ({ ...current, obrigatorio: e.target.checked }))} className="accent-accent" />Obrigatório</label>
              <button type="button" disabled={!canOperateStage || !newChecklist.titulo.trim() || savingItem} onClick={async () => { setSavingItem(true); await run(async () => { await rhJornadaService.createChecklistItem({ etapa_id: stage.id, titulo: newChecklist.titulo.trim(), descricao: newChecklist.descricao.trim() || null, link_url: newChecklist.link_url.trim() || null, obrigatorio: newChecklist.obrigatorio }); setNewChecklist({ titulo: '', descricao: '', link_url: '', obrigatorio: true }); await loadData(); await onStageUpdated?.(); }, { success: 'Item adicionado ao checklist.', error: 'Não foi possível adicionar o item ao checklist.' }); setSavingItem(false); }} className={cn('px-5 py-3 rounded-2xl font-black text-white flex items-center gap-2 transition-all', !canOperateStage || !newChecklist.titulo.trim() || savingItem ? 'bg-surface-3 opacity-60 cursor-not-allowed' : 'bg-accent hover:bg-accent')}><Plus className="w-4 h-4" />Adicionar</button>
            </div>
          </div>
        </Card>
          ...
          <Card className="p-5 border border-strong/50">
            ...
            <div className="space-y-3 mb-4">
              {responsibles.map((responsible) => (
                <div key={responsible.id} className="rounded-2xl border border-base bg-surface/30 p-4 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" disabled={!canAssignResponsibles} onClick={() => run(async () => { await rhJornadaService.setStageResponsibleAsPrimary(responsible.id); await loadData(); }, { success: 'Responsável principal definido.', error: 'Não foi possível definir o responsável principal.' })} className={cn('px-3 py-2 rounded-xl font-black text-xs transition-all', !canAssignResponsibles ? 'bg-surface-3 opacity-60 cursor-not-allowed text-secondary' : responsible.principal ? 'bg-success/15 text-success border border-success/30' : 'bg-surface-2 text-secondary hover:bg-surface-3')}>{responsible.principal ? 'Principal' : 'Tornar principal'}</button>
                ...
                <div key={document.id} className="rounded-2xl border border-base bg-surface/30 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {document.storage_path ? <button type="button" onClick={() => run(async () => { const url = await rhJornadaService.getDocumentSignedUrl(document.storage_path!); window.open(url, '_blank', 'noopener,noreferrer'); }, { error: 'Não foi possível abrir o documento.' })} className="px-3 py-2 rounded-xl border border-base bg-surface/40 text-secondary text-xs font-black hover:bg-surface/60 transition-all">Visualizar</button> : null}
                      <button type="button" disabled={!canOperateStage || uploadingDocumentId === document.id} onClick={() => { setPendingDocumentId(document.id); fileInputRef.current?.click(); }} className={cn('px-3 py-2 rounded-xl border border-base text-xs font-black transition-all', !canOperateStage || uploadingDocumentId === document.id ? 'bg-surface-3 opacity-60 cursor-not-allowed text-secondary' : 'bg-accent text-white hover:bg-accent')}>{document.storage_path ? 'Trocar arquivo' : 'Enviar arquivo'}</button>
