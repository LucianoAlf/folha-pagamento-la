                ...
                <div key={document.id} className="rounded-2xl border border-line bg-surface/30 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button type="button" disabled={!canOperateStage || uploadingDocumentId === document.id} onClick={() => { setPendingDocumentId(document.id); fileInputRef.current?.click(); }} className={cn('px-3 py-2 rounded-xl border border-line text-xs font-black transition-all', !canOperateStage || uploadingDocumentId === document.id ? 'bg-surface-3 opacity-60 cursor-not-allowed text-secondary' : 'bg-accent text-white hover:bg-accent')}>{document.storage_path ? 'Trocar arquivo' : 'Enviar arquivo'}</button>
