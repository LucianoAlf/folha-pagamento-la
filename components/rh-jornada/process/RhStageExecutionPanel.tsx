            ...
            {checklist.map((item) => (
              <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-4 space-y-3">
                <div className="flex items-start gap-3">
                  <input type="checkbox" checked={item.concluido} disabled={!canOperateStage} onChange={(e) => run(async () => { await rhJornadaService.toggleChecklistItem(item.id, e.target.checked); await loadData(); await onStageUpdated?.(); }, { error: 'Não foi possível atualizar o item do checklist.' })} className="accent-violet-500 mt-1" />
            ...
            <div className="space-y-3 mb-4">
              {responsibles.map((responsible) => (
                <div key={responsible.id} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-4 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" disabled={!canAssignResponsibles} onClick={() => run(async () => { await rhJornadaService.setStageResponsibleAsPrimary(responsible.id); await loadData(); }, { success: 'Responsável principal definido.', error: 'Não foi possível definir o responsável principal.' })} className={cn('px-3 py-2 rounded-xl font-black text-xs transition-all', !canAssignResponsibles ? 'bg-slate-700 opacity-60 cursor-not-allowed text-slate-300' : responsible.principal ? 'bg-emerald-500/15 text-emerald-200 border border-emerald-500/30' : 'bg-slate-800 text-slate-200 hover:bg-slate-700')}>{responsible.principal ? 'Principal' : 'Tornar principal'}</button>
                    <button type="button" disabled={!canAssignResponsibles} onClick={() => run(async () => { await rhJornadaService.removeStageResponsible(responsible.id); await loadData(); }, { success: 'Responsável removido.', error: 'Não foi possível remover o responsável.' })} className={cn('p-2 rounded-xl border border-rose-500/30 text-rose-200 transition-all', !canAssignResponsibles ? 'opacity-60 cursor-not-allowed bg-slate-900/30' : 'bg-rose-500/10 hover:bg-rose-500/20')}><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ))}
                ...
                <div key={document.id} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {document.storage_path ? <button type="button" onClick={() => run(async () => { const url = await rhJornadaService.getDocumentSignedUrl(document.storage_path!); window.open(url, '_blank', 'noopener,noreferrer'); }, { error: 'Não foi possível abrir o documento.' })} className="px-3 py-2 rounded-xl border border-slate-800 bg-slate-900/40 text-slate-200 text-xs font-black hover:bg-slate-900/60 transition-all">Visualizar</button> : null}
