        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <Card className="p-5 border border-slate-700/50">
              ...
              <div className="space-y-3">
                {documents.map((document) => <div key={document.id} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-white font-black">{document.titulo}</div><div className="mt-1 text-xs font-bold text-slate-400">{document.tipo_documento} • {document.categoria}</div></div><Badge variant={document.status === 'conferido' ? 'success' : document.status === 'rejeitado' ? 'danger' : 'warning'}>{document.status}</Badge></div>{document.storage_path ? <button type="button" onClick={async () => { const url = await rhJornadaService.getDocumentSignedUrl(document.storage_path!); window.open(url, '_blank', 'noopener,noreferrer'); }} className="mt-3 px-3 py-2 rounded-xl border border-slate-800 bg-slate-900/50 text-xs font-black text-slate-300 hover:bg-slate-900/70 transition-all">Visualizar</button> : null}</div>)}
