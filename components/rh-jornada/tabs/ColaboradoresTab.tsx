        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <Card className="p-5 border border-slate-700/50">
              ...
              <div className="space-y-3">
                {documents.map((document) => <div key={document.id} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-white font-black">{document.titulo}</div><div className="mt-1 text-xs font-bold text-slate-400">{document.tipo_documento} • {document.categoria}</div></div><Badge variant={document.status === 'conferido' ? 'success' : document.status === 'rejeitado' ? 'danger' : 'warning'}>{document.status}</Badge></div>{document.observacao ? <div className="mt-2 text-sm font-bold text-slate-300">{document.observacao}</div> : null}<div className="mt-3 flex flex-wrap gap-2">{document.storage_path ? <button type="button" onClick={() => run(async () => { const url = await rhJornadaService.getDocumentSignedUrl(document.storage_path!); window.open(url, '_blank', 'noopener,noreferrer'); }, { error: 'Não foi possível abrir o documento.' })} className="px-3 py-2 rounded-xl border border-slate-800 bg-slate-900/50 text-xs font-black text-slate-300 hover:bg-slate-900/70 transition-all">Visualizar</button> : null}<button type="button" onClick={() => run(async () => { await rhJornadaService.reviewCollaboratorDocument(document.id, document.status === 'conferido' ? 'em_analise' : 'conferido', document.observacao || null); await refreshSelected(); }, { success: document.status === 'conferido' ? 'Revisão reaberta.' : 'Documento conferido.', error: 'Não foi possível atualizar o documento.' })} className="px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black transition-all">{document.status === 'conferido' ? 'Reabrir revisão' : 'Marcar conferido'}</button><button type="button" onClick={() => { const confirmed = window.confirm(`Excluir o documento \"${document.titulo}\" do dossiê?`); if (!confirmed) return; return run(async () => { await rhJornadaService.deleteCollaboratorDocument(document.id); await refreshSelected(); }, { success: 'Documento excluído.', error: 'Não foi possível excluir o documento.' }); }} className="px-3 py-2 rounded-xl border border-rose-500/30 bg-rose-500/10 text-xs font-black text-rose-200 hover:bg-rose-500/20 transition-all">Excluir</button></div></div>)}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <Card className="p-5 border border-slate-700/50">
              ...
              <div className="space-y-3">
                {milestones.map((item) => <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-white font-black">{item.titulo}</div><div className="mt-1 text-xs font-bold text-slate-400">{item.tipo} • {new Date(item.created_at).toLocaleDateString('pt-BR')}</div></div><Badge variant={item.celebrado ? 'success' : 'warning'}>{item.celebrado ? 'Celebrado' : 'Pendente'}</Badge></div>{item.descricao ? <div className="mt-2 text-sm font-bold text-slate-300">{item.descricao}</div> : null}{!item.celebrado ? <button type="button" onClick={() => run(async () => { await rhJornadaService.celebrateMilestone(item.id); await refreshSelected(); }, { success: 'Marco celebrado.', error: 'Não foi possível celebrar o marco.' })} className="mt-3 px-3 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-black transition-all">Celebrar agora</button> : null}</div>)}
