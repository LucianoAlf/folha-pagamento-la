    ...
    <div className="space-y-6">
      ...
      <Card className="p-5 border border-line-strong/50">
        <div className="grid grid-cols-1 lg:grid-cols-[220px_220px_minmax(0,1fr)] gap-4">
          ...
          <div><div className="text-[10px] uppercase tracking-[0.2em] text-muted font-black mb-2">Busca</div><div className="relative"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por documento, processo ou pessoa" className="w-full rounded-2xl border border-line bg-bg px-5 py-3.5 text-sm font-bold text-secondary placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent/40" /><Filter className="w-4 h-4 absolute right-4 top-1/2 -translate-y-1/2 text-muted" /></div></div>
        </div>
      </Card>
      ...
      <Card className="p-5 border border-line-strong/50">
        ...
        {filteredDocuments.length === 0 ? <div className="rounded-3xl border border-dashed border-line-strong bg-surface/30 p-8 text-center"><div className="mx-auto w-14 h-14 rounded-3xl bg-surface-2/70 flex items-center justify-center mb-4"><FileWarning className="w-6 h-6 text-muted" /></div><div className="text-primary font-black">Nenhum documento encontrado</div><div className="mt-2 text-sm font-bold text-muted">Ajuste os filtros ou crie novos lancamentos documentais.</div></div> : <div className="space-y-3">{filteredDocuments.map((document) => { const canManage = document.origem === 'colaborador' ? isAdminOrRh(access.role) : canManageDocumentProcess(access, document.processo || null); const ownerLabel = document.colaborador?.nome || document.candidato?.nome || 'Sem vinculo'; const title = document.titulo_display || document.tipo_documento; return <div key={document.id} className="rounded-3xl border border-line bg-surface/30 p-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><div className="text-white text-lg font-black">{title}</div><Badge variant={STATUS_META[document.status].variant}>{STATUS_META[document.status].label}</Badge>{document.origem === 'colaborador' ? <Badge variant="purple">Dossie</Badge> : null}{document.obrigatorio ? <Badge variant="warning">Obrigatorio</Badge> : null}</div><div className="mt-2 text-sm font-bold text-muted">{document.processo?.titulo || document.categoria || 'Dossie do colaborador'} • {ownerLabel}</div><div className="mt-1 text-xs font-bold text-muted">{document.nome_arquivo || 'Nenhum arquivo enviado'} {document.enviado_em ? `• enviado em ${new Date(document.enviado_em).toLocaleDateString('pt-BR')}` : ''}</div>{document.observacao ? <div className="mt-3 rounded-2xl border border-line bg-bg/40 p-3 text-sm font-bold text-secondary">{document.observacao}</div> : null}</div><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => { setPendingUploadDoc(document); fileInputRef.current?.click(); }} disabled={!canManage || uploadingId === document.id} className={cn('px-4 py-2.5 rounded-2xl font-black text-white flex items-center gap-2 transition-all', !canManage || uploadingId === document.id ? 'bg-surface-2 text-muted border border-line cursor-not-allowed' : 'bg-accent hover:bg-accent text-white')}><UploadCloud className="w-4 h-4" />{document.storage_path ? 'Trocar arquivo' : 'Enviar arquivo'}</button><button type="button" onClick={() => setReviewing(document)} disabled={!canManage} className={cn('px-4 py-2.5 rounded-2xl border border-line bg-surface/40 text-secondary font-black flex items-center gap-2 transition-all', !canManage ? 'opacity-60 cursor-not-allowed' : 'hover:bg-surface/60')}><CheckCircle2 className="w-4 h-4" />Revisar</button>{document.storage_path ? <button type="button" onClick={() => run(async () => { const url = await rhJornadaService.getDocumentSignedUrl(document.storage_path!); window.open(url, '_blank', 'noopener,noreferrer'); }, { error: 'Não foi possível abrir o documento.' })} className="px-4 py-2.5 rounded-2xl border border-line bg-surface/40 text-secondary font-black hover:bg-surface/60 flex items-center gap-2 transition-all"><Eye className="w-4 h-4" />Visualizar</button> : null}</div></div></div>; })}</div>}
      </Card>
