            ...
            <Card className="p-5 border border-line-strong/50">
              ...
              <div className="space-y-3">
                {stages.map((stage) => (
                  <div
                    key={stage.id}
                    className={`rounded-2xl border p-4 ${selectedStageId === stage.id ? 'border-info/40 bg-info/10' : 'border-line bg-surface/30'}`}
                  >
                    <div className="grid grid-cols-1 md:grid-cols-[90px_minmax(0,1fr)_180px] gap-3">
                      <input
                        ...
                        className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-info/40"
                      />
                      <input
                        value={stage.titulo}
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3">
                      ...
                      <input
                        ...
                        className="min-w-[240px] flex-1 rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-info/40"
                      />
                      <input
                    ...
                    <div className="mt-3 grid grid-cols-1 xl:grid-cols-2 gap-3">
                      <textarea
                        ...
                        className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-info/40"
                      />
                      <textarea
              ...
              {selectedTemplate ? (
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-[1fr_220px] gap-3">
                  <CustomSelect
                    ...
                    options={ROLE_OPTIONS}
                  />
                  <label className="flex items-center gap-2 rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary">
                    <input
                      type="checkbox"
                    ...
                    <div key={document.id} className="rounded-2xl border border-line bg-surface/30 p-4">
                      <div className="grid grid-cols-1 md:grid-cols-[1fr_100px_auto] gap-3">
                        <input
                          ...
                          className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-warning/40"
                        />
                        <input
                    ...
                    <div key={item.id} className="rounded-2xl border border-line bg-surface/30 p-4 space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-[90px_1fr_180px_auto] gap-3">
                        <input
                          ...
                          className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-success/40"
                        />
                        <input
                          value={item.titulo}
                      ...
                      <textarea
                        ...
                        className="w-full rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary resize-none focus:outline-none focus:ring-2 focus:ring-success/40"
                      />
                      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3">
                        ...
                        {item.link_url ? (
                          <button
                            ...
                            className="px-4 py-3 rounded-2xl border border-line bg-surface/40 text-secondary font-black hover:bg-surface/60 flex items-center gap-2 transition-all"
                          >
          ...
          <div className="mt-8 space-y-6">
            <Card className="p-5 border border-line-strong/50">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-[300px_minmax(0,1fr)] gap-6">
                <div className="space-y-3">
                  {pdiTemplates.map((template) => <button key={template.id} type="button" onClick={() => setSelectedPdiTemplateId(template.id)} className={['w-full rounded-3xl border p-4 text-left transition-all', template.id === selectedPdiTemplateId ? 'border-accent/30 bg-accent/10' : 'border-line bg-surface/30 hover:bg-surface/50'].join(' ')}><div className="text-primary font-black">{template.nome}</div><div className="mt-1 text-xs font-bold text-muted">{template.escopo_cargo || 'Sem cargo'} • v{template.versao}</div><div className="mt-3"><Badge variant={template.ativo ? 'success' : 'default'}>{template.ciclo_tipo || 'personalizado'}</Badge></div></button>)}
                </div>

                {selectedPdiTemplate ? (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <input value={pdiTemplateForm.nome} onChange={(e) => setPdiTemplateForm((prev) => ({ ...prev, nome: e.target.value }))} className="w-full rounded-2xl border border-line bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" placeholder="Nome do template" />
                      <input value={pdiTemplateForm.escopo_cargo} onChange={(e) => setPdiTemplateForm((prev) => ({ ...prev, escopo_cargo: e.target.value }))} className="w-full rounded-2xl border border-line bg-bg px-5 py-3.5 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40" placeholder="Cargo alvo" />
                      <div className="md:col-span-2"><textarea value={pdiTemplateForm.descricao} onChange={(e) => setPdiTemplateForm((prev) => ({ ...prev, descricao: e.target.value }))} rows={3} className="w-full rounded-2xl border border-line bg-bg px-5 py-4 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-accent/40 resize-none" placeholder="Descrição do template" /></div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                      <Card className="p-5 border border-line-strong/50">
                        ...
                        <div className="space-y-3">
                          {pdiCompetences.map((item) => <div key={item.id} className="rounded-2xl border border-line bg-surface/30 p-4"><input value={item.nome} onChange={(e) => setPdiCompetences((prev) => prev.map((current) => current.id === item.id ? { ...current, nome: e.target.value } : current))} className="w-full rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-info/40" /><div className="mt-3 grid grid-cols-2 gap-3"><CustomSelect value={item.categoria} onValueChange={(value) => setPdiCompetences((prev) => prev.map((current) => current.id === item.id ? { ...current, categoria: value as typeof item.categoria } : current))} options={RH_PDI_COMPETENCE_CATEGORIES.map((value) => ({ value, label: value }))} /><input value={item.nivel_alvo} onChange={(e) => setPdiCompetences((prev) => prev.map((current) => current.id === item.id ? { ...current, nivel_alvo: Number(e.target.value) || current.nivel_alvo } : current))} className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-info/40" /></div><button type="button" onClick={() => run(async () => { await rhJornadaService.updatePdiTemplateCompetence(item.id, { nome: item.nome, categoria: item.categoria, nivel_alvo: item.nivel_alvo, ordem: item.ordem }); await refreshPdiTemplateDetails(selectedPdiTemplate.id); }, { success: 'Competência salva.', error: 'Não foi possível salvar a competência.' })} className="mt-3 px-4 py-2 rounded-2xl bg-info hover:bg-info text-white font-black transition-all">Salvar</button></div>)}
                          <div className="grid grid-cols-1 gap-3"><input value={newPdiCompetence.nome} onChange={(e) => setNewPdiCompetence((prev) => ({ ...prev, nome: e.target.value }))} placeholder="Nova competencia" className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-info/40" /><div className="grid grid-cols-2 gap-3"><CustomSelect value={newPdiCompetence.categoria} onValueChange={(value) => setNewPdiCompetence((prev) => ({ ...prev, categoria: value as typeof prev.categoria }))} options={RH_PDI_COMPETENCE_CATEGORIES.map((value) => ({ value, label: value }))} /><input value={newPdiCompetence.nivel_alvo} onChange={(e) => setNewPdiCompetence((prev) => ({ ...prev, nivel_alvo: e.target.value }))} placeholder="Nivel alvo" className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-info/40" /></div><button type="button" onClick={() => run(async () => { await rhJornadaService.createPdiTemplateCompetence({ template_id: selectedPdiTemplate.id, nome: newPdiCompetence.nome, categoria: newPdiCompetence.categoria, nivel_alvo: Number(newPdiCompetence.nivel_alvo || 3), ordem: pdiCompetences.length + 1 }); setNewPdiCompetence({ nome: '', categoria: RH_PDI_COMPETENCE_CATEGORIES[0], nivel_alvo: '3' }); await refreshPdiTemplateDetails(selectedPdiTemplate.id); }, { success: 'Competência adicionada.', error: 'Não foi possível adicionar a competência.' })} className="px-4 py-3 rounded-2xl bg-info hover:bg-info text-white font-black transition-all">Adicionar competencia</button></div>
                        </div>
                      </Card>
                      ...
                      <Card className="p-5 border border-line-strong/50">
                        ...
                        <div className="space-y-3">
                          {pdiCheckpoints.map((item) => <div key={item.id} className="rounded-2xl border border-line bg-surface/30 p-4"><input value={item.titulo} onChange={(e) => setPdiCheckpoints((prev) => prev.map((current) => current.id === item.id ? { ...current, titulo: e.target.value } : current))} className="w-full rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-warning/40" /><div className="mt-3 grid grid-cols-2 gap-3"><CustomSelect value={item.tipo} onValueChange={(value) => setPdiCheckpoints((prev) => prev.map((current) => current.id === item.id ? { ...current, tipo: value as typeof item.tipo } : current))} options={RH_PDI_CHECKPOINT_TYPES.map((value) => ({ value, label: value }))} /><input value={item.prazo_offset_dias || 0} onChange={(e) => setPdiCheckpoints((prev) => prev.map((current) => current.id === item.id ? { ...current, prazo_offset_dias: Number(e.target.value) || 0 } : current))} className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-warning/40" /></div><button type="button" onClick={() => run(async () => { await rhJornadaService.updatePdiTemplateCheckpoint(item.id, { titulo: item.titulo, tipo: item.tipo, prazo_offset_dias: item.prazo_offset_dias, objetivo_template_id: item.objetivo_template_id || null, ordem: item.ordem }); await refreshPdiTemplateDetails(selectedPdiTemplate.id); }, { success: 'Checkpoint salvo.', error: 'Não foi possível salvar o checkpoint.' })} className="mt-3 px-4 py-2 rounded-2xl bg-warning hover:bg-warning text-white font-black transition-all">Salvar</button></div>)}
                          <div className="grid grid-cols-1 gap-3"><input value={newPdiCheckpoint.titulo} onChange={(e) => setNewPdiCheckpoint((prev) => ({ ...prev, titulo: e.target.value }))} placeholder="Novo checkpoint" className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-warning/40" /><CustomSelect value={newPdiCheckpoint.objetivo_template_id} onValueChange={(value) => setNewPdiCheckpoint((prev) => ({ ...prev, objetivo_template_id: value }))} options={[{ value: '', label: 'Sem objetivo vinculado' }, ...pdiObjectives.map((item) => ({ value: item.id, label: item.titulo }))]} /><div className="grid grid-cols-2 gap-3"><CustomSelect value={newPdiCheckpoint.tipo} onValueChange={(value) => setNewPdiCheckpoint((prev) => ({ ...prev, tipo: value as typeof prev.tipo }))} options={RH_PDI_CHECKPOINT_TYPES.map((value) => ({ value, label: value }))} /><input value={newPdiCheckpoint.prazo_offset_dias} onChange={(e) => setNewPdiCheckpoint((prev) => ({ ...prev, prazo_offset_dias: e.target.value }))} placeholder="Prazo em dias" className="rounded-2xl border border-line bg-bg px-4 py-3 text-sm font-bold text-secondary focus:outline-none focus:ring-2 focus:ring-warning/40" /></div><button type="button" onClick={() => run(async () => { await rhJornadaService.createPdiTemplateCheckpoint({ template_id: selectedPdiTemplate.id, objetivo_template_id: newPdiCheckpoint.objetivo_template_id || null, titulo: newPdiCheckpoint.titulo, tipo: newPdiCheckpoint.tipo, ordem: pdiCheckpoints.length + 1, prazo_offset_dias: Number(newPdiCheckpoint.prazo_offset_dias || 30) }); setNewPdiCheckpoint({ objetivo_template_id: '', titulo: '', tipo: RH_PDI_CHECKPOINT_TYPES[0], prazo_offset_dias: '30' }); await refreshPdiTemplateDetails(selectedPdiTemplate.id); }, { success: 'Checkpoint adicionado.', error: 'Não foi possível adicionar o checkpoint.' })} className="px-4 py-3 rounded-2xl bg-warning hover:bg-warning text-white font-black transition-all">Adicionar checkpoint</button></div>
                        </div>
                      </Card>
                    </div>
                  </div>
